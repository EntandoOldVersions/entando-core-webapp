...
var Calendar = new Class({	
	...
	initialize: function(obj, options) {
		...
		for (var i in obj) {
			var cal = { 
				button: new Element('button', { 'type': 'button', 'title': this.options.calendarTitle }).addEvent("click",function(ev){ev.preventDefault()}),
				el: $(i),
				els: [],
				id: id++,
				month: d.getMonth(),
	...
	display: function(cal) {
		...
		var table = new Element('table',{
			"class": this.classes.tablecalendar,
			"cellpadding": "0",
			"cellspacing": "0"
		}).inject(div).adopt(this.caption(cal));
